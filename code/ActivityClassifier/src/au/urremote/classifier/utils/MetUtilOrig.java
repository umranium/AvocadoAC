...
/**
 * Assists in the computation of energy expenditure, and MET. As explained on
 * the paper: <a href=
 * "http://jap.physiology.org/content/83/6/2112.full.pdf#page=1&view=FitH">
 * Improving energy expenditure estimation by using a triaxial accelerometer</a>
 * 
 * <p>
 * This class assumes that the data is being sampled at 20Hz, hence some of the
 * computations done, are purposefully done for 20Hz only. One of the
 * computations is by getting the mean of every consequent group of 4 samples,
 * hence effectively reducing the sampling frequency to 5Hz. The max signal
 * frequency we can obtain from 5Hz sampling frequency this is 2.5Hz, which is
 * our required upper limit for our frequency filter. The lower limit is 0.25Hz,
 * which since we are sampling for too short a period, we can't detect.
 * </p>
 * 
 * @author abd01c
 * 
 */
public class MetUtilOrig implements OptionUpdateHandler {
	...
	private static final double[] DC_COMPONENTS = new double[] { 0.0, 0.0,
			Constants.GRAVITY };
	 ...
	 */
	public MetUtilOrig(double mass, double height, double age, int gender) {
		this.mass = mass;
		this.height = height;
		this.age = age;
		this.gender = gender;
	...
	private void computeRestingEE() {
		...
		if (gender == GENDER_MALE) {
			restingEE = 4.19 * ((473 * weightInLb) + (971 * heightInInches)
					- (513 * age) + 4687) / 100000.0;
		} else {
			restingEE = 4.19 * ((331 * weightInLb) + (352 * heightInInches)
					- (353 * age) + 49854) / 100000.0;
		}
	}
	 ...
	 * <p>
	 * <code>EEact = EE - EEresting</code> <code>MET = EE / EEresting</code>
	 * </p>
	 * 
	 * @param eeAct
	 *            The actual energy expenditure of the activity, computed using
	 *            the function
	...
	/**
	 * Computes the EEact given the two horizontal accelerations, and vertical
	 * accelerations.
	 * <p>
	 ...
	 * 
	 * @param counts
	 *            counts per second in all 3 accelerometer axi
	...
	public double computeEEact(double counts[]) {
		...
		double horCounts = Math.sqrt(counts[0] * counts[0] + counts[1]
				* counts[1]);
	 ...
	 * 
	 * @param len
	 *            the number of accelerometer samples in each dimension
	 * @param rotatedData
	 *            the accelerometer samples (3 dimensions, after rotation)
	 * @param timeStamps
	 *            timestamps of the accelerometer samples
	 * @param results
	 *            an array of three doubles to return the average number of
	 *            zero-crossings per second
	 */
	public void computeCountsPerSecond(int len, float[][] rotatedData,
			long[] timeStamps, double[] results) {
		...
		for (int dim = 0; dim < 3; ++dim) {
			...
			for (int iSample = 0, iGroup = 0; iSample + MEAN_GROUP_SIZE < len; iSample += MEAN_GROUP_SIZE, ++iGroup) {
				...
				} else if (beenToTheLowerBand && isInTheUpperBand) {
					++counts;
